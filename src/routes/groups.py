...
async def create_student(body: GroupModel, db: Session = Depends(get_db)):
    group = await  repository_group.create_group(body, db)
    return group


@router.get("/", response_model=List[GroupResponse], name="List of all groups", )
async def get_groups(request: Request, limit: int = Query(20, le=500), offset: int = 0,
    ...
    return templates.TemplateResponse('groups.html', {'request': request, 'groups': groups, 'title': 'Groups'})